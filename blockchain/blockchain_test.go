...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	engine := prometheus.New(params.TestnetChainConfig.Prometheus,bc.chainDb)
	ncm, err := NewBlockChain(bc.chainDb, bc.config, engine, vm.Config{})
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archiveDb, _ := hpbdb.NewMemDatabase()
	gspec.MustCommit(archiveDb)
	engine := prometheus.New(params.TestnetChainConfig.Prometheus,archiveDb)
	archive, _ := NewBlockChain(archiveDb, gspec.Config, engine, vm.Config{})
	defer archive.Stop()
	...
	fastDb, _ := hpbdb.NewMemDatabase()
	gspec.MustCommit(fastDb)
	engine := prometheus.New(params.TestnetChainConfig.Prometheus,fastDb)
	fast, _ := NewBlockChain(fastDb, gspec.Config, engine, vm.Config{})
	defer fast.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	var (
		...
		address  = crypto.PubkeyToAddress(key.PublicKey)
		funds    = big.NewInt(1000000000)
		gspec    = &Genesis{Config: params.TestnetChainConfig, Alloc: GenesisAlloc{address: {Balance: funds}}}
		genesis  = gspec.MustCommit(gendb)
	)
	height := uint64(1024)
	...
	engine = prometheus.New(params.TestnetChainConfig.Prometheus,lightDb)
	light, _ := NewBlockChain(lightDb, gspec.Config, engine, vm.Config{})
...
func TestLogReorgs(t *testing.T) {
	...
	var (
		...
		gspec   = &Genesis{Config: params.TestnetChainConfig, Alloc: GenesisAlloc{addr1: {Balance: big.NewInt(10000000000000)}}}
		genesis = gspec.MustCommit(db)
		signer  = types.NewEIP155Signer(gspec.Config.ChainId)
	)
	engine = prometheus.New(params.TestnetChainConfig.Prometheus,db)
	...
	chain, _ := GenerateChain(params.TestnetChainConfig, genesis, db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(params.TestnetChainConfig, genesis, db, 3, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(chain); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
