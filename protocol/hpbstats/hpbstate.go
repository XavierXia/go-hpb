...
// Package hpbstates implements the network stats reporting service.
package hpbstats
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.hpb != nil {
		...
		if block == nil {
			block = s.hpb.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.hpb.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
