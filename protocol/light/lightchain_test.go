...
func makeHeaderChain(parent *types.Header, n int, db hpbdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestnetChainConfig, types.NewBlockWithHeader(parent), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (hpbdb.Database, *LightChain, error) {
	db, _ := hpbdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestnetChainConfig}
...
func newTestLightChain() *LightChain {
	...
	engine := prometheus.New(params.TestnetChainConfig.Prometheus,db)
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, engine)
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() hpbdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestnetChainConfig, engine)
	if err != nil {
