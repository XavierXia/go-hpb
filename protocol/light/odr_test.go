...
type testOdr struct {
	OdrBackend
	sdb, ldb hpbdb.Database
	disable  bool
}

func (odr *testOdr) Database() hpbdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db hpbdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db hpbdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db hpbdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db hpbdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db hpbdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb, _  = hpbdb.NewMemDatabase()
		ldb, _  = hpbdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	engine := prometheus.New(params.TestnetChainConfig.Prometheus,sdb)
	blockchain, _ := core.NewBlockChain(sdb, params.TestnetChainConfig,engine, vm.Config{})
	gchain, _ := core.GenerateChain(params.TestnetChainConfig, genesis, sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb}
	engine = prometheus.New(params.TestnetChainConfig.Prometheus,ldb)
	lightchain, err := NewLightChain(odr, params.TestnetChainConfig, engine)
	if err != nil {
		t.Fatal(err)
	}
