...
import (
	...
	"github.com/hpb-project/go-hpb/network/p2p/discv5"
...
func NewLesServer(hpb *hpb.Hpb, config *hpb.Config) (*LesServer, error) {
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(hpb.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(hpb.ChainDb())
	return srv, nil
}
...
func newCostStats(db hpbdb.Database) *requestCostStats {
...
func getChtRoot(db hpbdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db hpbdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db hpbdb.Database) bool {
