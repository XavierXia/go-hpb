...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*hpbdb.MemDatabase).Keys() {
	...
	statedb, _ := hpbdb.NewMemDatabase()
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	var (
		...
		poa           = prometheus.New(config.Prometheus, db)
		genesis       = gspec.MustCommit(db)
		blockchain, _ = core.NewBlockChain(db, config, poa, vm.Config{})
	)
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, evmux, new(testTxPool), poa, blockchain, db)
	if err != nil {
	...
	peer, _ := newTestPeer("peer", hpb_v1, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
				block.SetExtra(common.FromHex("0x64616f2d686172642d666f726b"))
			}
		})
